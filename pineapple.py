...
class ResumeGenerator:
    ...
    def __init__(self, markdown_file: Path, output_file: Path | None = None):
        self.markdown_file = markdown_file
        self.output_file = output_file or markdown_file.with_suffix(".pdf")
        self.candidate_name: str | None = None
        self.candidate_title: str | None = None
        self.ai_metadata: str | None = None
    ...
    def parse_markdown(self) -> list:
        ...
        while i < len(lines):
            ...
            if line.lower().startswith("<!-- ai"):
                in_ai_block = True
                after = re.sub(
                    r"^<!--\s*ai\s*:?\s*", "", line, flags=re.IGNORECASE
                ).strip()
            ...
            if line.startswith("# "):
                ...
                if not passed_first_section:
                    ...
                    elements.append(
                        Paragraph(f"<b>{text}</b>", self.styles["Heading2"])
                    )
            ...
            # ── Bold standalone line ──────────────────────────────────────
                ...
                if in_video_section:
                    video_body.append(
                        Paragraph(f"<b>{text}</b>", self.styles["VideoBody"])
                    )
                else:
    ...
    def _try_role_header(self, title: str, meta_line: str) -> list | None:
    ...
    def _process_inline_markdown(self, text: str) -> str:
        ...
        text = re.sub(
            r"`([^`]+)`", r'<font face="Courier" color="#6B6B6B">\1</font>', text
        )
        return text
